...
class HeroInfo:
    ...
    def update(self, jsonpath, imagepath):
        heroes = {}
        data = json.loads(urlopen(HEROES_URL % dotaconfig.API_KEY).read())
    ...
    def update_hero(self, id, name, roles):
        parameters = ["carry" in roles, "disabler" in roles, "durable" in roles, "escape" in roles,
                      "initiator" in roles, "jungler" in roles, "lanesupport" in roles, "nuker" in roles,
                      "pusher" in roles, "support" in roles, "melee" in roles, "ranged" in roles, id]
        ...
        if self.cursor.rowcount == 0:
            self.cursor.execute(
                "INSERT INTO heroes(name,carry,disabler,durable,escape,initiator,jungler,lanesupport,nuker,pusher,support,melee,ranged,heroid) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                parameters)
        else:
            self.cursor.execute(
                "UPDATE heroes SET name = %s, carry = %s, disabler = %s, durable = %s, escape = %s, initiator = %s, jungler = %s, lanesupport = %s, nuker = %s, pusher = %s, support = %s, melee = %s, ranged = %s WHERE heroid = %s",
                parameters)
...
class MatchFetcher:
    ...
    def load_new_matches(self, userid):
        ...
        for matchid in matchids:
            try:
                ...
                print ("Error fetching match with id: %s: %s" % (matchid, e))
            except Exception as e:
                print ("Error parsing match with id: %s: %s" % (matchid, e))

    def load_new_matchids(self, userid):
        newmatches = []
        nexturl = MATCHES_URL % (dotaconfig.API_KEY, userid)
        print ("Fetching %s" % (nexturl))
        data = json.loads(urlopen(nexturl).read())["result"]
        while data["matches"]:
            last_match_id = 0
    ...
    def load_match(self, userid, matchid):
        ...
        captain = 'radiant_captain' in data and 'dire_captain' in data and (
                    data['radiant_captain'] == userid or data['dire_captain'] == userid)
        ...
        parameters = [matchid, userid, pdata["hero_id"], timestamp, data["duration"], pdata["kills"], pdata["deaths"],
                      pdata["assists"], pdata["level"], pdata["gold"] + pdata["gold_spent"] if ("gold" in pdata and "gold_spent" in pdata) else 0, pdata["last_hits"],
                      pdata["denies"], pdata["xp_per_min"], pdata["gold_per_min"], str(won), str(radiant), total_kills,
                      total_deaths, str(captain), max_level, pdata["hero_damage"] if "hero_damage" in pdata else 0, pdata["tower_damage"] if "tower_damage" in pdata else 0,
                      data["game_mode"]]

        self.cursor.execute(
            "INSERT INTO matches(matchid,userid,heroid,datetime,duration,kills,deaths,assists,level,gold,lasthits,denies,xp_min,gold_min,won,radiant,total_kills,total_deaths,captain,max_level,hero_damage,tower_damage,game_mode) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            parameters)
...
class DotaUser:
    ...
    def load_name(self):
        ...
        data = json.loads(urlopen(NAME_URL % (userid64, dotaconfig.API_KEY)).read())["response"]
...
if __name__ == '__main__':
    with closing(conn_db()) as conn:
        with closing(conn.cursor()) as cursor:
            ...
            group.add_argument("--update-heroes", nargs=2, metavar=("jsonpath", "imagepath"),
                               help="Updates the hero metadata and images")
